...
**\*\*\\\*\\\*Scope:\\\*\\\*\*\*** Card 04 — Stage-3 Provider Semantics Handoff

**\*\*\\\*\\\*Status:\\\*\\\*\*\*** Draft — minimum public interests, canonical mapping, Fatih-only destination scope, membership semantics, bounded-request evidence, and repository regression tests completed; production candidate ID policy frozen by Backend and exact Fatih boundary evidence recorded; final Backend handoff confirmation remains pending

**\*\*\\\*\\\*Revision:\\\*\\\*\*\*** Coordinated mapping, Fatih scope, membership evidence, and bounded-request update — 17 September 2026
...
\\\* Provider \\\`place\\\_id\\\` is source identity, not proof of unique physical-place identity.
...
\\\* Different provider \\\`place\\\_id\\\` values.
...
\\\* 20/20 bounded-request records matching the required \\\`country\\\_code=tr\\\`, \\\`city=Istanbul\\\`, \\\`town=Fatih\\\`, and valid-coordinate checks.
...
**\*\*### 2.4 Remaining Narrow Destination Evidence\*\***

The destination scope and routing are confirmed. The remaining narrow evidence item is approval/reference capture for the specific Fatih boundary \\\`place\\\_id\\\` used by the bounded provider request. The bounded request and observed validation are recorded in Section 15.
...
The verified planner/FastAPI snapshot is:
...
The current planner/FastAPI contract does not define a fixed canonical allowlist. Verified Stage 2A examples used IDs such as \\\`historic\\\_site\\\`, \\\`museum\\\`, and \\\`monument\\\`, but those examples do not define the complete taxonomy.

**\*\*### 3.3 Proposed Public Interest → Canonical Mapping\*\***

AI/Data proposes the following evidence-backed mapping for review:

\\| Confirmed public App Interest ID | Proposed canonical category IDs sent to \\\`/plan\\\` | Evidence basis | Status |

\\| --- | --- | --- | --- |

\\| \\\`history\\\` | \\\`historic\\\_site\\\` | Current evidence contains archaeological sites, memorial/historic evidence, ruins, and corroborated historic places | Accepted by TL and Backend for the minimum Stage 3 scope |
...
This proposal does not map \\\`history\\\` to \\\`museum\\\`, because the inspected provider evidence does not establish a museum mapping. It does not map generic sightseeing buildings or squares to \\\`monument\\\` without explicit accepted evidence.
...
The current development fixtures use \\\`history\\\` and \\\`landmark\\\` in \\\`categoryIds\\\`. Those values came from the Iteration 2 development evidence and must not be treated as frozen planner taxonomy.

The affected Stage 3/provider clean fixtures and provenance have been migrated consistently:

\\- Development \\\`history\\\` becomes canonical \\\`historic\\\_site\\\` where the recorded evidence supports the historic-site rule.
...
Repository regression tests were run against the current task working tree:
...
\\\`culture\\\`, \\\`art\\\`, \\\`nature\\\`, \\\`adventure\\\`, and \\\`food\\\` remain outside the confirmed minimum Stage 3 provider mapping in this package. The public contract/UI must handle them according to the team's release decision. They must not be silently mapped to \\\`historic\\\_site\\\` or \\\`monument\\\`.

**\*\*\\\*\\\*Current status:\\\*\\\*\*\*** the minimum public IDs, mapping ownership, exact public-to-canonical mapping, and conservative Geoapify guidance are accepted by Ahmad and Mohammad for the minimum Stage 3 scope. Provider fixtures were migrated and repository regression tests passed (\\\`83 passed\\\`). Final Backend handoff confirmation remains pending.
...
The table below translates the existing evidence into the canonical IDs \\\`historic\\\_site\\\` and \\\`monument\\\` accepted by Ahmad and Mohammad for the minimum Stage 3 scope.

\\| Observed Geoapify evidence | Proposed canonical \\\`categoryIds\\\` | Guidance / limitation |

\\| --- | --- | --- |

\\| \\\`tourism.sights.archaeological\\\_site\\\` | \\\`historic\\\_site\\\` | Supported by the reviewed archaeological-site evidence |

\\| \\\`tourism.sights.memorial\\\` | \\\`historic\\\_site\\\` | Treat as historic evidence under the current proposal; do not infer \\\`monument\\\` without explicit monument evidence |

\\| \\\`tourism.sights.memorial.monument\\\` | \\\`historic\\\_site\\\`, \\\`monument\\\` | Supports both the history and landmark public flows |

\\| \\\`tourism.sights.ruines\\\` | \\\`historic\\\_site\\\` | Existing development proposal; preserve Geoapify's observed spelling \\\`ruines\\\` |

\\| \\\`tourism.sights.building\\\` with corroborating historic evidence | \\\`historic\\\_site\\\` | Record-specific historic evidence is required; generic building alone is insufficient |

\\| \\\`tourism.sights.building\\\` alone | No accepted mapping | Do not infer \\\`historic\\\_site\\\` or \\\`monument\\\` |
...
Once the release mapping table is agreed, Backend should:
...
**\*\*\\\*\\\*Proposed conservative release policy:\\\*\\\*\*\*** hold records affected by an unresolved subtype decision outside the eligible pool until the relevant parent/subtype behavior is agreed.

Record the accepted behavior explicitly, including whether any independently supported category can still make such a record eligible.

**\*\*### 4.4 Mapping Review Decision Required\*\***

Ahmad and Mohammad should confirm whether:

\\- \\\`historic\\\_site\\\` and \\\`monument\\\` are acceptable canonical IDs for this release implementation;

\\- the public mappings in Section 3.3 are correct;

\\- the provider rules above produce the same canonical IDs expected by Backend and \\\`/plan\\\`;

\\- any rule needs a narrower condition before fixture migration.

Ahmad and Mohammad accepted this mapping for the minimum Stage 3 scope. Preserve the reviewed document/commit reference and repository validation evidence in the final handoff.

**\*\*## 5. Destination Membership Semantics — Minimum Fatih Policy Proposed\*\***
...
This policy does not claim support for all Istanbul. The existing fixture value \\\`istanbul-tr\\\` remains a development identifier until the current public/Backend destination ID is confirmed.
...
The exact Fatih boundary \\\`place\\\_id\\\` is not preserved in the current committed request evidence. This is a precise evidence gap, not a reason for a broad provider research rerun.
...
2\\. \\\`properties.country\\\_code\\\`, normalized by trimming and case-folding, equals \\\`tr\\\`.
...
5\\. Latitude and longitude are present, numeric, finite, and within valid ranges.
...
1\\. A mismatch in \\\`country\\\_code\\\`, \\\`city\\\`, or \\\`town\\\` makes the record out of scope, even if the boundary-filtered request returned it.

2\\. A missing \\\`country\\\_code\\\`, \\\`city\\\`, or \\\`town\\\` makes membership unknown.

3\\. \\\`formatted\\\`, \\\`address\\\_line2\\\`, \\\`postcode\\\`, and \\\`suburb\\\` are audit/supporting metadata only; they do not override a mismatch or missing required signal.
...
1\. Exact Geoapify Fatih boundary \`place\_id\` recorded:

   \`510de9a683abf23c40598128f3ea77824440f00101f901d8f21a0000000000c002089203054661746968\`.

2\. Sanitized geocoding evidence preserves the Geoapify endpoint, request parameters with \`apiKey=YOUR\_API\_KEY\`, retrieval timestamp, and matching Fatih response feature.
...
**\*\*Review status:\*\*** Fatih-only membership semantics are frozen for the minimum Stage 3 scope, the exact boundary evidence is complete, and only Mohammad's final Backend handoff confirmation remains pending.
...
Mohammad accepted this minimum policy in Backend review. Its implementation still depends on the finalized membership mechanism, category rules, and canonical ID policy; the acceptance does not resolve those dependencies.
...
If the team chooses to allow them, document that alternative explicitly, including its interaction with selected interests and existing planner behavior. Do not change planner ranking or fallback behavior implicitly through this data handoff.

**\*\*\\\*\\\*Decision status:\\\*\\\*\*\*** accepted in principle by Backend; final handoff confirmation remains pending until dependent mapping and membership decisions are complete.
...
\\| Provider \\\`place\\\_id\\\` | Identity of a source record at Geoapify |
...
Backend must document how canonical IDs are assigned and how they remain traceable to provider records.
...
\\\* Repeated observations with the same provider ID require an explicit repeated-record policy.
...
\\\* Use the Geoapify \`place\_id\` as the source identity input.

\\\* Generate the canonical candidate ID using the provider namespace plus the provider ID:

  \`geoapify:\<place\_id>\`

\\\* The same Geoapify \`place\_id\` encountered more than once within the same candidate-building operation must resolve to the same canonical candidate ID.

\\\* Identical repeated records with the same provider \`place\_id\` should produce one candidate while preserving traceability to the repeated source observations.

\\\* If repeated records with the same provider \`place\_id\` contain conflicting canonical-relevant data, they must not be silently merged. The candidate remains blocked until the conflict is handled explicitly.

\\\* Different Geoapify \`place\_id\` values remain different canonical candidate IDs even when names or coordinates are similar.
...
\\\* **\*\*\\\*\\\*Milion:\\\*\\\*\*\*** explicit memorial/monument evidence; migrated from the earlier development labels to canonical \\\`historic\\\_site\\\` and \\\`monument\\\`.

\\\* **\*\*\\\*\\\*Archeological Park:\\\*\\\*\*\*** archaeological-site mapping to development category \\\`history\\\`.
...
\\| Image | \\\`wiki\\\_and\\\_media.image\\\` observed in some Fatih records; not observed in the inspected Rome sample | Do not assume universal coverage or that this is the provider’s only possible image source |
...
**\*\*## 13. Implementation-Ready Handoff to Mohammad — Acceptance Pending\*\***
...
\\| Verified planner contract | Snapshot \\\`7f41755a7744721421471c93dff526be5496ff6e\\\`; no fixed canonical allowlist | Confirmed contract behavior | Ahmad; Section 3.2 |

\\| Canonical category IDs | \\\`historic\\\_site\\\`, \\\`monument\\\` | Accepted for minimum Stage 3 scope | Ahmad + Mohammad reviews; Sections 3.3 and 4 |

\\| Public Interest → canonical mapping | \\\`history\\\` → \\\`historic\\\_site\\\`; \\\`landmark\\\` → \\\`monument\\\` | Accepted for minimum Stage 3 scope | Ahmad + Mohammad reviews; Section 3.3 |

\\| Other current public IDs | Outside minimum Stage 3 mapping | Documented scope limitation | Section 3.5 |

\\| Provider category rules and subtype handling | Mapping to \\\`historic\\\_site\\\` / \\\`monument\\\` in Section 4 | Accepted for minimum Stage 3 scope | Ahmad + Mohammad reviews; unresolved subtype remains unmapped |

\\| Membership mechanism and conflict handling | Fatih-only executable rule in Section 5 | Accepted for minimum Stage 3 scope | Exact boundary \\\`place\\\_id\\\` approval/reference remains the narrow gap |
...
\\| Canonical ID assignment | Deterministic provider-namespaced ID \`geoapify:\<place\_id>\`; identical repeated provider IDs collapse to one candidate with preserved traceability; conflicting repeats are not silently merged | Accepted and frozen by Backend | Section 9.3; Mohammad final review confirmation |
...
\\| Optional metadata | Optional and not fabricated | Documented; final review pending | Section 11 |
...
**\*\*\\\*\\\*Current confirmation status: First Backend review completed; final handoff confirmation pending.\\\*\\\*\*\***
...
The earlier mapping, public-destination, membership, and repository-test blockers have now been closed for the minimum Stage 3 scope through the recorded team coordination and regression evidence.

Before final confirmation, the remaining narrow item is:

1\\. Send the updated package to Mohammad and capture his final Backend handoff confirmation that no blocking semantic question remains for mapping, membership, or eligibility in the minimum supported flow.
...
\\| Document version / commit SHA | Local coordination revision; repository SHA pending |

\\| Relevant Backend baseline | Pending |
...
\\| Remaining blocking questions | Final Backend handoff confirmation only |

\\| Final outcome | Pending — implementation-ready confirmation not yet granted |

\\| Final confirmation comment / PR link | Pending |

**\*\*### 14.3 Final Confirmation Template\*\***

After the blockers are resolved, request this confirmation:

\\> I reviewed Stage 3 semantics package [version/SHA] and its referenced decisions and fixtures. For the agreed supported scope, I have sufficient guidance to implement mapping, destination membership, and eligibility without unresolved blocking semantic questions.

\\>

\\> I acknowledge the documented limitations, including physical-place deduplication limits, optional metadata gaps, and disabled Budget support.

\\>

\\> Remaining non-blocking limitations: [list or “none identified”].

\\>

\\> Remaining blocking questions: [list or “none”].

\\>

\\> Review reference and date: [record].

**\*\*\\\*\\\*Card 04 is complete only when the remaining decisions are recorded, the updated package is delivered, and Mohammad’s final confirmation is captured.\\\*\\\*\*\***
...
ID, minimum destination scope, and membership semantics. The exact boundary identifier is now supported by sanitized geocoding evidence. Final Backend handoff confirmation remains pending.
...
\\- Require \\\`country\\\_code=tr\\\`, \\\`city=Istanbul\\\`, and \\\`town=Fatih\\\`.
...
\\- 19 records with a supported \\\`historic\\\_site\\\` mapping.
...
\- Bounded request evidence: captured and reviewed at report level.
...
\- Exact Fatih boundary identifier evidence/reference: Complete — sanitized Geoapify geocoding request/response recorded for the exact \`place\_id\`; Ahmad's coordination message identified this as the final boundary-evidence item.

\- Final Backend handoff confirmation: Pending — updated package ready for Mohammad's final review.

\- Reviewer / confirmation link / date: Pending.

The provider fixture migration and planner/model regression suite passed without failures. The production canonical candidate ID policy is accepted and frozen by Backend, and the exact Fatih boundary \`place\_id\` now has sanitized geocoding evidence plus bounded-request evidence. The remaining handoff work is limited to obtaining Mohammad's final confirmation that no unresolved blocking semantic questions remain for mapping, membership, or eligibility in the minimum supported flow.